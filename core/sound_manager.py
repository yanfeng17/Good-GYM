    ...
    def init_sounds(self):
        ...
        try:
            ...
            if os.path.exists(docker_assets):
                print(f"[SoundManager] OK Docker环境检测到", flush=True)
            ...
            print(f"[SoundManager] ERR 初始化失败: {e}", flush=True)
            import traceback
            traceback.print_exc()
            sys.stdout.flush()
        ...
        for tool in tools:
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"[SoundManager] OK 本地播放工具: {tool}", flush=True)
                    break
            except:
                pass
        
        if not self.audio_tool:
            print("[SoundManager] WARN 未找到本地音频播放工具", flush=True)
    ...
    def _send_ws_audio_event(self, sound_type, count=None):
        ...
        def send_async():
            try:
                ...
                sock.close()
                print(f"[SoundManager-WS] OK 发送音频事件: {sound_type} (count={count})", flush=True)
                
            except ConnectionRefusedError:
                print(f"[SoundManager-WS] WARN WebSocket服务器未就绪", flush=True)
            except Exception as e:
                print(f"[SoundManager-WS] WARN 发送失败: {e}", flush=True)
        ...
        def play_async():
            try:
                ...
                if result.returncode != 0 and result.stderr:
                    print(f"[SoundManager] WARN 本地播放警告: {result.stderr[:200]}", flush=True)
                    
            except subprocess.TimeoutExpired:
                print(f"[SoundManager] WARN 播放超时", flush=True)
            except Exception as e:
                print(f"[SoundManager] ERR 播放异常: {e}", flush=True)
